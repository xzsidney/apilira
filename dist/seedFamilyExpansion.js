...
async function seedFamilyExpansion() {
    ...
    try {
        ...
        await models_1.FamilyLocation.sync({ alter: false });
        await models_1.FamilyActiveMission.sync({ alter: false });
        await models_1.FamilyStoryAdventure.sync({ alter: false });
        await models_1.FamilyStoryNode.sync({ alter: false });
        await models_1.FamilyStoryChoice.sync({ alter: false });
        await models_1.FamilyAchievement.sync({ alter: false });
        console.log('✅ Novas tabelas da Família sincronizadas com sucesso.');
        // 1. Locais do Radar (Casa e Vizinhança)
        const locCount = await models_1.FamilyLocation.count();
        if (locCount === 0) {
            console.log('🌱 Criando Locais do Radar da Família Lira...');
            await models_1.FamilyLocation.bulkCreate([
                {
                    id: 'loc_cozinha',
        ...
        const achCount = await models_1.FamilyAchievement.count();
        if (achCount === 0) {
            console.log('🌱 Criando Conquistas e Medalhas da Família Lira...');
            await models_1.FamilyAchievement.bulkCreate([
                {
        ...
        const advCount = await models_1.FamilyStoryAdventure.count();
        if (advCount === 0) {
            console.log('🌱 Criando 1º Livro-Jogo Solo da Família Lira...');
            const adv = await models_1.FamilyStoryAdventure.create({
            ...
            // Nó 1: Introdução
            const node1 = await models_1.FamilyStoryNode.create({
                adventureId: adv.id,
                nodeId: 'node_intro',
            ...
            // Escolhas Nó 1
            await models_1.FamilyStoryChoice.bulkCreate([
                {
            ...
            await models_1.FamilyStoryNode.create({
                adventureId: adv.id,
        ...
        const { FamilyCharacter } = await Promise.resolve().then(() => __importStar(require('./models')));
