...
function generateMatrixForZone(zoneName: string): MatrixMissionTemplate[] {
  return [
    // 💪 1. PILAR FÍSICO
    {
      pilar: 'FÍSICO',
...
async function seedNineMatrixMissions() {
  try {
    ...
    for (const eq of equipmentList) {
      let existing = await DefinitionEquipment.findOne({ where: { name: eq.name } });
      if (!existing) {
        await DefinitionEquipment.create(eq as any);
        console.log(`🛡️ Equipamento cadastrado: ${eq.name}`);
      }
    }

    // 2. Busca todas as Zonas Principais (Level 2) e Bairros (Level 3)
    const zones = await DefinitionLocation.findAll({ where: { level: 2 } });
    ...
    for (const zone of zones) {
      ...
      const districtChildren = await DefinitionLocation.findAll({ where: { parentId: zone.id } });
