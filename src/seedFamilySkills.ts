...
export const CLASS_SKILLS_DATA = [
  // --- CURANDEIRA ✨ ---
  {
...
async function seedFamilySkills() {
  ...
  try {
    ...
    await FamilyCharacter.sync({ alter: true });
    await FamilyClassSkill.sync({ alter: true });
    await FamilyCharacterSkill.sync({ alter: true });
    console.log('✅ Tabelas family_class_skills e family_character_skills sincronizadas.');

    // Popula as habilidades da árvore
    for (const skill of CLASS_SKILLS_DATA) {
      const [record, created] = await FamilyClassSkill.findOrCreate({
        where: { id: skill.id },
        defaults: skill,
      });
      if (!created) {
    ...
    console.log(`✅ ${CLASS_SKILLS_DATA.length} Habilidades cadastradas com sucesso.`);

    // Desbloqueia automaticamente a habilidade Grau I correspondente à classe de cada herói existente
    const characters = await FamilyCharacter.findAll();
