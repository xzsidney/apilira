...
interface DistrictLore {
  huntTitle: string;
  huntDesc: string;
  huntAttr: string;
  huntSkill: string;
  huntSuccessText: string;
  huntFailureText: string;
  opTitle: string;
  opDesc: string;
  opAction1: { name: string; desc: string; attr: string; skill: string; diff: number; success: string; fail: string };
  opAction2: { name: string; desc: string; attr: string; skill: string; diff: number; success: string; fail: string };
  opAction3: { name: string; desc: string; attr: string; skill: string; diff: number; success: string; fail: string };
}

function getDistrictLore(bairro: any): DistrictLore {
...
async function seedAllDistrictMissions() {
  try {
    ...
    for (const district of districts) {
      ...
      let huntMission = await DefinitionMissionIdle.findOne({
        where: { locationId: district.id, category: 'HUNT' }
      });

      if (!huntMission) {
        huntMission = await DefinitionMissionIdle.create({
      ...
      let opMission = await DefinitionMissionIdle.findOne({
        where: { locationId: district.id, category: 'OPERATION' }
      });

      if (!opMission) {
        opMission = await DefinitionMissionIdle.create({
          locationId: district.id,
          title: lore.opTitle,
    ...
    const existingStoryCount = await DefinitionStoryAdventure.count();
    if (existingStoryCount <= 1) {
      ...
      const adv1 = await DefinitionStoryAdventure.create({
      ...
      const n1_1 = await DefinitionStoryNode.create({
        adventureId: adv1.id,
      ...
      const n1_success = await DefinitionStoryNode.create({
        adventureId: adv1.id,
      ...
      const n1_fail = await DefinitionStoryNode.create({
        adventureId: adv1.id,
      ...
      const adv2 = await DefinitionStoryAdventure.create({
      ...
      const n2_1 = await DefinitionStoryNode.create({
        adventureId: adv2.id,
      ...
      const n2_success = await DefinitionStoryNode.create({
        adventureId: adv2.id,
      ...
      const n2_fail = await DefinitionStoryNode.create({
        adventureId: adv2.id,
