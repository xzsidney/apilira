...
async function seedFamilyData() {
  ...
  try {
    ...
    await FamilyCharacter.sync({ alter: false });
    await FamilyTask.sync({ alter: false });
    await FamilyTaskLog.sync({ alter: false });
    await FamilyBattle.sync({ alter: false });
    await FamilyBattleParticipant.sync({ alter: false });
    await FamilyShopItem.sync({ alter: false });
    console.log('✅ Tabelas da Família sincronizadas com sucesso.');

    // 1. Seed dos 7 Personagens da Família
    const charactersCount = await FamilyCharacter.count();
    ...
    const tasksCount = await FamilyTask.count();
    if (tasksCount === 0) {
    ...
    const battleCount = await FamilyBattle.count();
    if (battleCount === 0) {
      console.log('🌱 Criando a Batalha Piloto contra o Golem da Bagunça...');
      const allMembers = await FamilyCharacter.findAll({ order: [['orderIndex', 'ASC']] });
      const turnOrder = allMembers.map(m => m.id);
      turnOrder.push('MONSTER');

      await FamilyBattle.create({
